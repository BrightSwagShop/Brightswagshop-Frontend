...
async function uploadResults() {
  try {
    ...
    const testRunResponse = await makeRequest('POST', `/projects/${projectId}/test-runs`, {
      test_run: {
        name: BUILD_NAME,
        description: `Automated run for ${BUILD_NAME}`,
      }
    });
    ...
    const folderId = await getOrCreateFolder(projectId, 'Frontend E2E');
    ...
    for (const testCase of testCases) {
      try {
        ...
        const batch = { results: [ { test_case_id: testCaseId, test_result: testResult } ] };
